...
all_chemicals = ["NO2"] # "HCHO" ,"SO2", "NO2", "CO"] ## "CH4"
...
for chemical in all_chemicals:
    ...
    if chemical == "NO2":
        ...
        derived = kernel_process(derived, 30, normalize=False)
        derived = normalize(derived)
        derived = np.power(10, derived)
        # derived = np.power(10, derived)
    elif chemical == "HCHO":
        derived = normalize(derived)
        # derived[derived > 0.3*np.nanmax(derived)] = np.nan
        
        derived = land_mask(derived, lon_min, lon_max, lat_min, lat_max)
        derived = kernel_process(derived, 60, normalize=False)
        derived = normalize(derived)
        derived = np.power(10, derived)
    elif chemical == "SO2":
        derived = normalize(derived)
    ...
    data_rigs = load_json("results/PLATFORMS_with_FIRMS_observations.json")
    rig_lats = [entry["coordinates"][1] for entry in data_rigs]
    rig_lons = [entry["coordinates"][0] for entry in data_rigs]
